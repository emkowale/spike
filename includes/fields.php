...
function spike_state_select($selected='MI'){
  $s=spike_states(); $h='<div class="field"><label for="state">State <span>*</span></label><select id="state" name="state" required>';
  foreach($s as $k=>$v){ $sel=($k===$selected)?' selected':''; $h.='<option value="'.$k.'"'.$sel.'>'.$v.'</option>'; }
  return $h.'</select></div>';
}
